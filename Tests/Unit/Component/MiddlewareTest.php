...
class MiddlewareTest extends TestCase
{
     ...
     * @test
     */
    public function middlewareWithoutBeforeAfterOmitsThoseKeys(): void
    {
        $middleware = new Middleware('Vendor\\Extension\\');
        $middleware->setName('SimpleMiddleware');
        $middleware->setIdentifier('vendor/extension/simple-middleware');
        $middleware->setType('frontend');

        $expected = [
            'target' => 'Vendor\\Extension\\SimpleMiddleware',
        ];

        self::assertEquals($expected, $middleware->getArrayConfiguration());
    }
}
